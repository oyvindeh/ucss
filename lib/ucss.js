...
var ucss = {
    /**
     * Find CSS selectors in a CSS file
     ...
     */
    _findSelectors: function(css, selectors) {
        if (!css) return {};
        ...
        // Add each matched selector, and count occurences
        while ((selector = pattern.exec(css))) {
            if (selector && selector[1]) {
                var r = selector[1].trim();
                if (undefined === selectors[r]) {
                    selectors[r] = 1;
                } else {
                    selectors[r]++;
                }
    ...
    search: function(css, html, cookie, whitelist, donecb) {
        ...
        self._getCssSelectors(css, function(selectors, result) {
            self._processHtml(self, html, cookie, selectors, whitelist, result, donecb);
        });
    },
     ...
     *         arguments, one set of selectors and one set of results (see
            ...
            if (0 === item.indexOf("http")) { // From URL
                ...
                require(protocol).get(options, function(res) {
                    ...
                    res.on('data', function (chunk) {
                        ...
                        matchedSelectors = ucss._findSelectors(data, matchedSelectors);
                        callback();
                    });
                });
                ...
                matchedSelectors = ucss._findSelectors(item, matchedSelectors);
            } else { // From string
                matchedSelectors = ucss._findSelectors(item, matchedSelectors);
            }
     ...
     */
    _processHtml: function(self, html, cookie, selectors, whitelist, result, donecb) {
            ...
            jsdom.env({
                ...
                headers: { 'Cookie': cookie },
                    ...
                    for (var i=0; i<selectors.length; i++) {
                        ...
                        if (whitelist && -1 < whitelist.indexOf(selector)) continue;
                        if (-1 < selector.indexOf("@")) continue;
                        ...
                        if (selector) {
                            ...
                            try {
                                if ($(selector).length > 0) {
                                    result.used[oSelector] = result.used[oSelector]
                                        + $(selector).length;
                                }
                            } catch (e) {
                                console.log("Problem with selector: " + oSelector);
                            }
                        }
                    }
                    callback();
...
module.exports = {
    /**
     * Analyze CSS: Find number of times a selector has been used, and if there are
     * duplicates.
     *
