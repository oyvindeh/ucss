...
var ucss = {
     ...
     *         object as argument.
     */
    search: function(css, item, donecb) {
        ...
        async.forEach(item, function(item, callback) {
            var html, cookie;
            if (undefined !== item.html) {
                html = item.html;
            } else {
                html = item;
            }

            if (undefined !== item.cookie) {
                cookie = item.cookie;
            } else {
                cookie = "";
            }
...
module.exports = {
     ...
     */
    analyze: function(css, html, auth, done) {
        ...
        if (auth) {
            ...
            loginFunc(loginUrl, username, password, function(cookie) {
                ...
                ucss.search(css, items, done);
            });
        } else {
            ucss.search(css, html, done);
        }
    }
};
