    ...
    _extractSelectors: function(css, selectors) {
        ...
        var _shake = function(rules) {
            for (var i=0; i<rules.length; i++) {
                ...
                // Rules without selectorText are ignored (@-rules, except @media)
                } else if (!rule.selectorText) {
                    selectors[rule.cssText] = 'ignored';
                ...
                headers: { 'Cookie': cookie },
                    ...
                    for (var i=0; i<selectors.length; i++) {
                        ...
                        if (-1 < selector.indexOf("@")) {
                            // For some selectors, like @font-face and @keyframes, there
                            // might be a lot of stuff that we don't need
                            var sel = selector.split("{")[0].trim();
                            result.ignored[sel] = 1;
                            continue;
                        }
