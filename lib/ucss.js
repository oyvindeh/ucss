...
var ucss = {
   ...
   * @param {Function} doneCallback Function to execute when done (typically a
   *                 reporter). An object on the form { "<selector>": count }
   *                 is passed to it, where count is the number of occurences of
   *                 <selector>.
   */
  search: function (pages, css, context, timeout, logger, doneCallback) {
    ...
    Q.fcall(function () {
      return cssHandler.getSelectors(css, whitelist, result, headers, timeout, logger);
    })
      .then(function (result) {
        if (result.total === 0) {
          throw 'No CSS loaded, nothing to do.';
        }
        if (pages) {
          return htmlHandler.matchSelectors(pages, result, headers, cookie,
                                            timeout, logger);
        }
        return result;
      })
      ...
      .done(function () {
        ...
        for (var selector in result.selectors) {
          ...
          if (sel.matches_html === 0 && !sel.ignored && !sel.whitelisted) {
            result.total_unused++;
          }

          // Ignored
          if (sel.ignored || sel.whitelisted) {
...
module.exports = {
   ...
   * @param {Object} context
   * @param {Array}  context.whitelist List of selectors to ignore
   * @param {Object} context.headers Headers to send to server
  ...
  analyze: function (pages, css, context, logger, doneCallback) {
    ...
    context.headers = context.headers ? context.headers : {};
