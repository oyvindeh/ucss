...
var ucss = {
    ...
    _extractSelectors: function(css, selectors) {
        ...
        var tree = styles.cssRules;

        var _shakeTree = function(tree) {
            for (var i=0; i<tree.length; i++) {
                var rule = tree[i];
                if (rule.cssRules) {
                    selectors = _shakeTree(rule.cssRules);
                } else {
                    var sl = rule.selectorText;

                    // A selector can be a comma separated list of selectors,
                    // e.g. ".foo, .bar":
                    var list = sl.split(",");
