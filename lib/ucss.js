...
var ucss = {
    ...
    search: function(context, doneCallback) {
        ...
        self.css.getSelectors(self.css, css, function(selectors, result) {
...
ucss.css = {
     ...
     */
    getSelectors: function(self, css, selectorsCollectedCallback) {
        ...
        async.forEach(css, function(item, forEachCallback) {
            if (0 === item.indexOf("http")) { // From URI
                ...
                request.get(options, function(error, res, data) {
                        matchedSelectors = self._extractSelectorsFromString(
                ...
                matchedSelectors = self._extractSelectorsFromString(self, item, matchedSelectors);
            } else { // From string
                matchedSelectors = self._extractSelectorsFromString(self, item, matchedSelectors);
            }
