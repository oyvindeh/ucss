...
var ucss = {
     ...
     */
    _matchSelectorsInHtml: function(self, domain, html, cookie, selectors,
        ...
        var _processHtmlString = function(html, currentUri, callback) {
            ...
            if (links.length) {
                for (var i=0; i<links.length; i++) {
                    ...
                    if (-1 === processed.indexOf(hash2)) {
                        if (0 === link.indexOf(domain)) { // current domain
                            q.push(link);
                        } else if (0 === link.indexOf("http")) { // another
                            // Skip, another domain
                        } else {
                            if (currentUri) {
                                q.push(url.resolve(currentUri, link));
                            } else {
        ...
        // To be called for each item in items (async).
        // Will do http requests to fetch html, if necessary.
        var _processItem = function(item, callback) {
            var html = item.html ? item.html : item;
            var cookie = item.cookie ? item.cookie : "";

            // Keep track of what has already been processed. This is used
            // when crawling, to see if a link has already been visited (both
            // logged in and not).
            var hash = crypto.createHash('md5').update(html + cookie).digest("hex");
            ...
            if (0 === html.indexOf("http")) { // From URI
                var uri = html;
                ...
                require('request').get(options, function(error, res, data) {
                    ...
                    _processHtmlString(data, uri, callback);
                });
            } else if (-1 === html.indexOf("<html>")) { // From file
                try {
                    html = fs.readFileSync(html).toString();
                } catch (e) {
                    console.log(e.message);
                }
                _processHtmlString(html, null, callback);
            } else { // From string
                _processHtmlString(html, null, callback);
            }
        };

        // Async processing of html items
        var q = async.queue(_processItem, 8);
        q.push(items);
    ...
    search: function(context, donecb) {
        ...
        self._collectCssSelectors(css, function(selectors, result) {
            self._matchSelectorsInHtml(self, domain, html, cookie, selectors,
                              whitelist, result, donecb);
        });
    }
};
...
module.exports = {
    ...
    analyze: function(context, done) {
        if (!(context.html instanceof Array)) {
