...
var ucss = {
     ...
     */
    _extractSelectorsFromCssString: function(self, css, selectors) {
        if (!selectors) {
            selectors = {};
        }
        if (!css) {
        ...
        selectors = self._getSelectorsFromRules(self, rules, selectors);
     ...
     */
    _collectCssSelectors: function(self, css, selectorsCollectedCallback) {
        ...
        async.forEach(css, function(item, forEachCallback) {
            if (0 === item.indexOf("http")) { // From URI
                ...
                request.get(options, function(error, res, data) {
                        matchedSelectors = ucss._extractSelectorsFromCssString(
                            self, data, matchedSelectors);
                        forEachCallback();
                });
                ...
                matchedSelectors = ucss._extractSelectorsFromCssString(self, item, matchedSelectors);
            } else { // From string
                matchedSelectors = ucss._extractSelectorsFromCssString(self, item, matchedSelectors);
            }
            ...
            if (selectorsCollectedCallback) {
                selectorsCollectedCallback(selectors, result);
            }
        });
    },
    ...
    _matchSelectorsInHtml: function(self, pages, cookie, selectors,
        ...
        var q = async.queue(function(item, queueCallback) {
    ...
    _processHtmlString: function(self, html, followLinks, currentUri, selectors, whitelist, cookie, result, processed, q, queueCallback) {
        ...
        if (followLinks) { // look for links in document, add to queue
            ...
            if (links.length) {
                for (var i=0, l=links.length; i<l; i++) {
                    self._handleLink(links[i], processed, currentUri, followLinks, q, cookie);
                }
            }
        }

        // Process current document
        self._matchSelectorsInHtmlString(document, selectors, whitelist, result, queueCallback);
    },

    _handleLink: function(link, processed, currentUri, followLinks, q, cookie) {
        ...
        if (-1 === processed.indexOf(hash2)) { // If not processed yet, process
            if (0 === link.indexOf(currentUri)) { // current domain
                ...
                if (cookie) {
                    q.push({page: link, followLinks: followLinks, cookie: cookie});
                }
    ...
    // TODO: Rename. Add docstring. Clean up parameters.
    _matchSelectorsInHtmlString: function(htmlString, selectors, whitelist, result, queueCallback) {
    ...
    search: function(context, doneCallback) {
        ...
        self._collectCssSelectors(self, css, function(selectors, result) {
