...
module.exports = {
     ...
     * @param {Boolean} silent Silent mode, only output rule list
     * @param {Boolean} showDuplicates Show duplicate rules
     * @param {Boolean} gotHtml Show duplicate rules
     */
  report: function (result, showFull, silent, showDuplicates, gotHtml) {
    ...
    if (!silent) {
      ...
      if (result.load_errors.length > 0) {
        console.log('\n' + RED + BOLD + 'WARNING: ' + RESET
                            + 'Some resources were not loaded successfully. This '
                            + 'affects the result. Please see crawler log for '
                            + 'more information.');
      }
    }
  },
     ...
     */
  logger: function (res, reqHref, loggedIn, error) {
    var outputStr = '';
    var statusStr = '';
    var md5sum = '';

    if (error) {
      outputStr = RED + BOLD + error + ': ' + RESET + reqHref;
    } else {
