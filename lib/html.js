...
module.exports = {
     ...
     *                                         duplicates: { ".foo": 2 } }
     */
    matchSelectors: function(pages, selectors, cookie, whitelist, timeout) {
    ...
    _getHtmlAsString: function(page, cookie, timeout) {
        ...
        if (0 === page.indexOf("http")) { // From URI
            ...
            request.get(options, function(error, res, data) {
                ...
                console.log(self._formatConsoleOutputForRequest(res, uri, loggedIn));
     ...
     * @returns String
     */
    _formatConsoleOutputForRequest: function(res, uri, loggedIn) {
        var outputStr = "";
        var statusStr = "";
        ...
        if (res.request.href === uri) {
            ...
            outputStr = util.format("HTTP %s %s " + YELLOW + "=>" + RESET + " %s",
                                    statusStr, uri, res.request.href);
        }
        if (loggedIn) {
