...
/**
 * Format the console output for a request, with colors.
 *
 * @private
 * @param {Object}  res Response object, as given by the request module.
 * @param {String}  uri The URI visited
...
module.exports = {
     ...
     *                                         duplicates: { ".foo": 2 } }
     */
    matchSelectors: function(pages, result, cookie, whitelist, timeout, customLogger) {
    ...
    _processHtmlString: function(html, context, queue) {
        ...
        return this._matchSelectorsInString(document, result, whitelist);
    },
     ...
     */
    _matchSelectorsInString: function(document, result, whitelist) {
        // Loop through selectors
        for (var selector in result.foundSelectors) {
...
buster.log(selector);
                if (-1 < selector.indexOf("@")) {
                    result.foundSelectors[selector].ignored = true;
                ...
                try {
                    ...
                    if (len > 0) {
                        ...
                        if (result.foundSelectors[oSelector].matches_html == 0) {
                            result.total_used++;
                        }

                        // Add number of matches for selector to total matches
                        result.foundSelectors[oSelector].matches_html =
                            result.foundSelectors[oSelector].matches_html + len;
                    }
                } catch (e) {
