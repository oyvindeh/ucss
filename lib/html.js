...
//            console.log("Visiting ", item.page.split("?")[0]);
            ...
            var visits = [];
            visits.push(self._getHtmlAsString(item, "", processed)); // regular visit
            if (cookie) {
            ...
            .spread(function(regularResult, loggedInResult) {
                ...
                self._processHtmlString(regularItem, selectors, whitelist, result,
                                        processed, queue, host, excludedSubdomains);
                ...
                if (loggedInResult) {
                    ...
                    self._processHtmlString(loggedInItem, selectors, whitelist, result,
                                            processed, queue, host, excludedSubdomains);                        }
            }).then(queueCallback)
            .done(); // Passing queue callback, in case it needs to abort
        }, 8);
    ...
    _processHtmlString: function(item, selectors, whitelist, result, processed, queue, host, excludedSubdomains) {
        var html = item.page;
...
// TODO: Move check for external links in here too?
        ...
        if (followLinks) { // look for links in document, add to queue
            ...
            for (var i=0, l=links.length; i<l; i++) {
                ...
                if (handleThis) {
                    this._handleLink(links[i].attribs.href, host, processed, uri, followLinks, queue);
                }
            }
        }
    ...
    _handleLink: function(link, host, processed, uri, followLinks, queue) {
// TODO: Can host be used instead of currentUri?
