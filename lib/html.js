...
module.exports = {
    ...
    matchSelectors: function(pages, cookie, selectors, whitelist) {
        ...
        var queue = async.queue(function(item, queueCallback) {
            ...
            var pagesToVisit = [];
            pagesToVisit.push(self._getHtmlAsString(item, null)); // regular visit
            if (cookie) {
                pagesToVisit.push(self._getHtmlAsString(item, cookie)); // logged in visit
            }
    ...
    _processHtmlString: function(item, context, queue) {
        ...
        if (followLinks) { // look for links in document, add to queue
            ...
            for (var i=0, l=links.length; i<l; i++) {
                ...
                if (handleThis) {
                    this._addLinkToQueue(links[i].attribs.href, queue, uri, followLinks);
                }
            }
        }
    ...
    /**
     * Add link to queue, given it's on the current domain.
     *
     * @param {String} link Link to handle.
     * @param {Object} queue Queue object, as returned from the Async library.
     * @param {String} parentUri URL to document that contained link.
     * @param {Boolean} followLinks Follow links contained in the document
     *                 link refers to.
     */
    ...
    _getHtmlAsString: function(item, cookie) {
        var deferred = Q.defer();
        var page = item.page;
        var data;
