...
module.exports = {
        ...
        var queue = async.queue(function(item, queueCallback) {
            setImmediate(function() {
                queueItemHandler(item, processed, result, headers, timeout,
                                 cookie, excludedSubdomains, queue,
                                 queueCallback);
            });
        }, 8);
     ...
     *                 be visited both with and without the cookie set.
     * @param {Int}    timeout Request timeout
     * @param {Array}  excludedSubdomains List of sub domains to be excluded
     *                 when crawling.
    ...
    _queueItemHandler: function(item, processed, result, headers, timeout,
        ...
        var processHtmlString = this._processHtmlString.bind(this);
        Q.spread(htmlToCheck, function(regularResult, loggedInResult) {
            ...
            if (regularResult) {
                processHtmlString(regularResult, context, queue);
            }

            if (loggedInResult) {
                processHtmlString(loggedInResult, context, queue);
            }
        }).fail(function(error) {
    ...
    _processHtmlString: function(html, context, queue) {
        ...
        // Match selectors in current string
        this._matchSelectorsInString(document, result);
    },
