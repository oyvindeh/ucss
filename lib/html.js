...
module.exports = {
  ...
  matchSelectors: function (pages, result, headers, cookie, timeout, logger) {
    ...
    if (pages.exclude) {
      for (i = 0, l = pages.exclude.length; i < l; i++) {
        ...
        if (current.test) {
          // regex detected
          excludedSubfolders.push(current);
          continue;
  ...
  _queueLinks: function (document, queue, context) {
    ...
    for (var i = 0, l = links.length; i < l; i++) {
      ...
      this._addLinkToQueue(link.attribs.href, queue, uri, followLinks, excludedSubfolders);
    }
  },
   ...
   *                  be followed?
   */
  _addLinkToQueue: function (link, queue, parentUri, followLinks, excludedSubfolders) {
