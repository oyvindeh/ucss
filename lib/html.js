...
module.exports = {
     ...
     */
    matchSelectors: function(pages, result, cookie, timeout, logger) {
        ...
        var queue = async.queue(function(item, queueCallback) {
            ...
            var pagesToVisit = [];
            pagesToVisit.push(self._getHtmlAsString(page, result, null, timeout)); // regular visit
            if (cookie) {
                pagesToVisit.push(self._getHtmlAsString(page, result, cookie, timeout)); // logged in visit
            }
            processed.push(page);
     ...
     */
    _getHtmlAsString: function(page, result, cookie, timeout) {
        var deferred = Q.defer();
        var data;
