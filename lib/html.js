...
module.exports = {
   ...
   *                 checked.
   * @param {String} pages.exclude Pages or subfolders to exclude.
  ...
  matchSelectors: function (pages, result, headers, cookie, timeout, logger) {
    if (!pages) return;
    if (!(pages.crawl || pages.exclude || pages.include)) return;
    ...
    if (pages.exclude) {
      for (i = 0, l = pages.exclude.length; i < l; i++) {
        ...
        if (current.test) {
          // regex detected
          excludedUrls.push(current);
          continue;
        } else if (current.indexOf('*') !== -1) {
          // folder globs
          var subfolder = current.substring(0, current.length - 1);

          excludedUrls.push(subfolder);
          continue;
        }
        excludedUrls.push(current.split('?')[0]);
      }
    }
    ...
    if (pages.crawl) {
      for (i = 0, l = pages.crawl.length; i < l; i++) {
        var page = pages.crawl[i];

        // Strip away parameters and fragments when crawling
        page = page.split('#')[0];
        page = page.split('?')[0];
   ...
   *                 be visited both with and without the cookie set.
   * @param {Int}    timeout Request timeout
   * @param {Array}  excludedUrls List of sub domains to be excluded
   *                 when crawling.
   ...
   * @param {Array}   context.excludedUrls List of subfolders to ignore
  ...
  _queueLinks: function (document, queue, context) {
    var uri = context.uri;
    var excludedUrls = context.excludedUrls;
    ...
    for (var i = 0, l = links.length; i < l; i++) {
      ...
      var protocol = url.parse(href).protocol ? url.parse(href).protocol : 'http:';
      ...
      this._addLinkToQueue(link.attribs.href, queue, uri, followLinks, excludedUrls);
    }
  },
   ...
   *                  be followed?
   */
  _addLinkToQueue: function (link, queue, parentUri, followLinks, excludedUrls) {
    ...
    if (url.parse(link).host !== host) handleThis = false;
    if (handleThis) {
      for (var j = 0; j < excludedUrls.length; j++) {
        var excluded = excludedUrls[j];
        if (excluded.test) {
          // is a regex
          if (excluded.test(link)) {
            handleThis = false;
            break;
          }
        } else if (link.indexOf(excluded) === 0 || url.parse(link).pathname.indexOf(excluded) === 0) {
          handleThis = false;
          break;
        }
      }
      ...
      if (selector) {
        ...
        try {
          ...
          console.log(RED + 'Selector: "' +
                      oSelector + '" has ' + error.name + '. ' + error.message + RESET);
        }
      }
    }
    return;
  ...
  _getHtmlAsString: function (page, result, headers, cookie, timeout) {
    ...
    if (0 === page.indexOf('http')) { // From URI
      ...
      try {
        ...
        result.load_errors.push({ uri: uri, error: error.message });
      }
      deferred.resolve(data);
    } else { // From string
